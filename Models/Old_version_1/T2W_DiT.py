...
class T2W_DiT(nn.Module):
    def __init__(self, D_in: int, N_walks: int, seg_per_walk: int, L_traj: int, d_context: int, n_layers: int, T: int):
        super().__init__()
        self.D_in = D_in
        ...
        self.walks_proj = nn.Sequential(
            ...
            Rearrange("(B N) D L", "B (N L) D", N=N_walks),
        )
        ...
        self.stages = SequentialWithAdditionalInputs(*[
            Block(N_walks, 256, 256, 256, 256, 8, dropout=0.15)
            for _ in range(n_layers)
        ])

        # (B, N, L, 128)
        self.head = nn.Sequential(
            Rearrange("B (N L) D",  "B N L D", N=N_walks),
            nn.Linear(256, 64),
            Swish(),
            nn.Linear(64, D_in)
        )
