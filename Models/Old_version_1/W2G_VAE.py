...
class W2G_VAE(nn.Module):
    def __init__(self, N_walks: int, seg_per_walk: int, N_interp: int, threshold: float=0.5):
        super().__init__()
        self.N_walks = N_walks
        self.seg_per_walk = seg_per_walk
        self.N_interp = N_interp
        self.threshold = threshold

        self.N_segs = N_walks * seg_per_walk
        ...
        self.encoder = nn.Sequential(
            ...
            *[SERes1D(128, 256, 128) for _ in range(3)],
            ...
            Rearrange("(B N_routes) D L_route", "B (N_routes L_route) D", N_routes=N_walks),

            *[AttentionBlock(256, 32, 512, 256, 8) for _ in range(2)],

            Rearrange("B (N_routes L_route) D", "B N_routes L_route D", N_routes=N_walks),
        )
